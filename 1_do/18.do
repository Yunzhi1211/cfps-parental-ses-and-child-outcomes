/*==================================================
  CFPS 2018 Intergenerational Matching + Outlier Cleaning
...
*--- Raw data paths
local adult   "${RAW}\cfps2018person_202512.dta"
local famecon "${RAW}\cfps2018famecon_202101.dta"
...
flowappend, year(2018) step("Raw adult sample")
...
keep ///
    pid pid_f pid_m fid18 ///
    party ///
    age gender ///
    cfps2018eduy incomea ///
    urban18 provcd18 ///
    qc201 qea1 qea2 ///
    qga1 qga101 ///
    qu1m qu201 ///
...
if _rc {
    egen info = rownonmiss( ///
        pid_f pid_m fid18 ///
        party age gender cfps2018eduy incomea urban18 provcd18 ///
        qc201 qea1 qea2 qga1 qga101 qu1m qu201 ///
        qn8011 qn8012 qn12012 qn12016 ///
        qp202 qp702 qq4010 qn4004 ///
        pt1 qs1001 qs502 qs801_b_2code ///
        qg303code_isco qg303code_isei qg303code_siops qg303code_egp )
...
flowappend, year(2018) step("Valid unique respondents")
...
preserve
    ...
    flowappend, year(2018) step("Intergenerational link identified")
restore

gen ex = qp702*60 if qp702>=0
label var ex "Weekly exercise duration (minutes)"

*—— Child basic characteristics
rename age              child_age
rename gender           child_gender
rename cfps2018eduy     child_edu
rename incomea          child_income
rename urban18          child_urban
rename provcd18         child_province
...
if _rc {
    egen info = rownonmiss( ///
        qg303code_isco qg303code_isei qg303code_siops qg303code_egp ///
        age gender cfps2018eduy incomea party urban18 provcd18 ///
        qc201 qea1 qea2 qga1 qga101 qu1m qu201 ///
        qn8011 qn8012 qn12012 qn12016 ///
        qp202 qp702 qq4010 qn4004 qs801_b_2code )
    bysort pid (info): keep if _n == _N
    drop info
}
isid pid
...
rename cfps2018eduy     f_edu
rename incomea          f_income
rename urban18          f_urban
rename provcd18         f_province
...
if _rc {
    egen finfo = rownonmiss( ///
        ...
        f_health_change f_exercise f_sleep f_religion f_job_asp )
...
rename cfps2018eduy     m_edu
rename incomea          m_income
rename urban18          m_urban
rename provcd18         m_province
...
if _rc {
    egen minfo = rownonmiss( ///
        ...
        m_health_change m_exercise m_sleep m_religion m_job_asp )
...
flowappend, year(2018) step("Matched to at least one parent")
...
preserve
    ...
    rename familysize18    family_size
    rename fswt_natcs18n   family_cs
...
preserve
    ...
    flowappend, year(2018) step("Matched to family economic data")
restore
...
preserve
    ...
    flowappend, year(2018) step("With child occupational outcome")
restore
...
compress
flowappend, year(2018) step("Yearly file saved")
save "${YOUT}\18_data.dta", replace
